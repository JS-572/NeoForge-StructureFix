...
public class LunarSaplingBlock extends BushBlock implements BonemealableBlock {
    public static final MapCodec<LunarSaplingBlock> CODEC = simpleCodec(LunarSaplingBlock::new);

    public LunarSaplingBlock(Properties properties) {
        super(properties);
    }

    @Override
    public MapCodec<LunarSaplingBlock> codec() {
        return CODEC;
    }
    ...
    private void growTree(ServerLevel level, BlockPos pos) {
        ...
        // Simple example tree:
        for (int i = 0; i < 6; i++) {
            level.setBlock(pos.above(i), ModBlocks.LUNAR_LOG.get().defaultBlockState(), 3);
        }
                ...
                for (int z = -4; z <= 4; z++) {
                    if (Math.abs(x) + Math.abs(y) + Math.abs(z) < 4) {
                        level.setBlock(top.offset(x, y, z),
                                ModBlocks.LUNAR_LEAVES.get().defaultBlockState(), 3);
                    }
                }
            }
        }
